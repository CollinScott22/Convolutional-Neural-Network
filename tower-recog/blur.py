...
def rotate(degree):
	for index, name in enumerate(classes):  
		...
		for img_name in os.listdir(class_path):  
			...
			fff = Image.new('RGBA', (32,32), color='white')
	...
	for index, name in enumerate(classes):
		...
		for img_name in os.listdir(class_path):  
			if(img_name == '.DS_Store'):
				continue
			img_path = class_path + img_name
			blur_name = img_name[:-4] + '_blur' + '.PNG'
			img = Image.open(img_path) 
			img_blur = img.filter(ImageFilter.GaussianBlur(radius=2))
			img_blur.save(output_path + '/' + name + '/' + blur_name)

def create_record():
	writer = tf.python_io.TFRecordWriter("tower_train.tfrecords")
	for index, name in enumerate(classes):  
		class_path = output_path +"/"+ name +"/"
		for img_name in os.listdir(class_path):  
			if(img_name == '.DS_Store'):
			...
			example = tf.train.Example(
				features = tf.train.Features(
					feature = {
						"label": tf.train.Feature(int32_list=tf.train.Int32List(value=[index])),#0 = Box, 1 = Turtle, 2 = Fiver
